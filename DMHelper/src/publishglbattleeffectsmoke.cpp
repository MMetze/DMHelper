...
static const char *smokeFragmentShader = "#version 410 core\n"
    ...
    "    float n1a = snoise(vec3(sUV1 * 1.4, u_time * 0.10 * churn)) * 0.5 + 0.5;\n"
    "    float n1b = snoise(vec3(sUV2 * 1.4, u_time * 0.10 * churn)) * 0.5 + 0.5;\n"
    "    float nLow = mix(n1a, n1b, w);\n"
    "    float n2a = snoise(vec3(sUV1 * 2.8, u_time * 0.18 * churn)) * 0.5 + 0.5;\n"
    "    float n2b = snoise(vec3(sUV2 * 2.8, u_time * 0.18 * churn)) * 0.5 + 0.5;\n"
    "    float nMid = mix(n2a, n2b, w);\n"
    "    float n3a = snoise(vec3(sUV1 * 5.2, u_time * 0.26 * churn)) * 0.5 + 0.5;\n"
    "    float n3b = snoise(vec3(sUV2 * 5.2, u_time * 0.26 * churn)) * 0.5 + 0.5;\n"
