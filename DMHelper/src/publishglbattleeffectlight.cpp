...
static const char *lightFragmentShader = "#version 410 core\n"
    ...
    "        float r1 = fract(sin((stepIndex + 13.1 + u_seed) * 12.9898) * 43758.5453);\n"
    "        float r2 = fract(sin((stepIndex + 14.1 + u_seed) * 12.9898) * 43758.5453);\n"
    "        float randomWave = mix(r1, r2, smoothstep(0.0, 1.0, stepPhase)) * 2.0 - 1.0;\n"
    "        float harmonic = sin(u_time * u_flickerFrequency * 11.37 + 0.73 + u_seed) * 0.35;\n"
