...
static const char *sparksFragmentShader = "#version 410 core\n"
    ...
    "        float seedA = hash11(fi * 3.17 + u_seed);\n"
    "        float seedB = hash11(fi * 7.13 + u_seed);\n"
    "        float seedC = hash11(fi * 11.91 + u_seed);\n"
    "\n"
    ...
    "\n"
    "        // Pseudo-perspective arc with proper ballistics: forge sparks launch upward, fall under\n"
    "        // gravity, bounce on the ground, then roll along their flight direction.\n"
    "        if(u_arcFalloff > 0.5) {\n"
    "            // Foreshorten the lateral travel slightly to fake a low camera pitch.\n"
    "            sparkPos.y *= 0.62;\n"
    "            float t = radial;\n"
    "            // Per-spark launch velocity (screen-up = +Y) and gravity. Higher speed = stronger gravity.\n"
    "            float launchV = (0.55 + speed * 0.30) * (0.80 + seedA * 0.50);\n"
    "            float gravity = 2.6 + speed * 0.9;\n"
    "            float y = launchV * t - 0.5 * gravity * t * t;\n"
    "            float t1 = 2.0 * launchV / max(0.0001, gravity); // first ground contact\n"
    "            float rollPhase = 0.0;\n"
    "            if(t > t1) {\n"
    "                // Bounce: reduced upward velocity, same gravity. After second ground contact, settle.\n"
    "                float vBounce = launchV * 0.38;\n"
    "                float dt = t - t1;\n"
    "                float yB = vBounce * dt - 0.5 * gravity * dt * dt;\n"
    "                float t2 = 2.0 * vBounce / max(0.0001, gravity);\n"
    "                y = (dt > t2) ? 0.0 : max(0.0, yB);\n"
    "                rollPhase = clamp((t - t1) / max(0.001, 1.0 - t1), 0.0, 1.0);\n"
    "            }\n"
    "            float arcAmp = (0.55 + seedC * 0.45) * (0.95 + speed * 0.20);\n"
    "            sparkPos.y += y * arcAmp;\n"
    "            // Ground roll: after first bounce, particles slide further along their launch direction.\n"
    "            sparkPos += dir * rollPhase * (0.10 + speed * 0.18) * (0.55 + seedB * 0.55);\n"
    "        }\n"
    "\n"
