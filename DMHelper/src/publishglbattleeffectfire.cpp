...
static const char *fireFragmentShader = "#version 410 core\n"
    ...
    "    // Per-instance domain offset so different fire effects don't share an identical noise path.\n"
    "    vec2 seedOffset = vec2(cos(u_seed), sin(u_seed * 1.37)) * 4.0;\n"
    "    float seedTime = u_seed * 0.61;\n"
    "    sUV1 += seedOffset;\n"
    "    sUV2 += seedOffset;\n"
    "\n"
    "    float n1a = snoise(vec3(sUV1 * freq,        u_time * flickerRate        + seedTime)) * 0.5 + 0.5;\n"
    "    float n1b = snoise(vec3(sUV2 * freq,        u_time * flickerRate        + seedTime)) * 0.5 + 0.5;\n"
    "    float nLow = mix(n1a, n1b, w);\n"
    "    float n2a = snoise(vec3(sUV1 * freq * 2.1,  u_time * flickerRate * 1.7  + seedTime)) * 0.5 + 0.5;\n"
    "    float n2b = snoise(vec3(sUV2 * freq * 2.1,  u_time * flickerRate * 1.7  + seedTime)) * 0.5 + 0.5;\n"
    "    float nMid = mix(n2a, n2b, w);\n"
    "    float n3a = snoise(vec3(sUV1 * freq * 4.3,  u_time * flickerRate * 2.5  + seedTime)) * 0.5 + 0.5;\n"
    "    float n3b = snoise(vec3(sUV2 * freq * 4.3,  u_time * flickerRate * 2.5  + seedTime)) * 0.5 + 0.5;\n"
    ...
    "    float flickerRaw = 0.85 + 0.18 * sin(u_time * u_flickerSpeed * 11.0 + u_seed)\n"
    "                              * sin(u_time * u_flickerSpeed * 6.7 + 1.3 + u_seed);\n"
    "    float pulseRaw   = 0.90 + 0.18 * sin(u_time * u_flickerSpeed * 5.3 + noise * 2.1 + u_seed);\n"
